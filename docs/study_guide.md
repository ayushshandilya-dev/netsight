...
RUN TIME (offline, any machine):
  ...
  LIVE → live_sniffer.py (Scapy sniff on a network interface) →
         SAME ingest_packet/finalize_window → bit-identical windows → real-time
...
| `active_defense.py` | RESPONSE center: MITRE intel, firewall rules, honeypot DNAT, blast-radius + auto-approve sim |
| `forensics_report.py` | SHA-256 Merkle-chain ledger + SOC incident PDF (fpdf2) |
...
### SOC Command Center (six workflow centers)
- **Command overview** — telemetry strip (threats / incidents / windows /
  flag rate / peak risk, all real), an inline-SVG **network-activity map**
  (nodes sized by flagged windows, edges weighted by family), **threat +
  incident queues** (the threat queue pre-selects a window in INVESTIGATION),
  and a **event workflow** timeline (INGEST → FORECAST → ALERT → THREAT →
  CORRELATE → AUDIT) whose stages are driven by the real run + ledger.
- **🔭 DETECTION** (ex-Forecaster) — risk timeline + alert table + RF/LSTM
  picker + threshold slider + PCAP upload. Button-triggered **RF-vs-LSTM
  side-by-side comparison** (agreement metrics + comparison chart) + threat
  matrix + attack narrative Gantt.
- **🔬 INVESTIGATION** (ex-Explainability + What-If Lab) — driving features
  (RF mean-imputation ablation), LSTM gradient saliency, model comparison, then
  a counterfactual **what-if**: perturb real alert-window features and re-run
  the model.
- **⚡ RESPONSE** (ex-Active Defense) — MITRE ATT&CK intel, generated firewall
  rules (iptables / netsh / Cisco ACL), honeypot DNAT. **SOAR blast-radius
  simulation**: per-family scoped containment with an **auto-approve toggle** —
  ON runs an autonomous staged timeline (inspect → contain → block → honey),
  OFF shows the guardrail "awaiting analyst approval". A hosts slider
  simulates blast-radius scope.
- **🌐 INTELLIGENCE** — cross-day per-family detection (DDoS 269/278 · Botnet
  16/92 · PortScan 0/351), **operating points** (precision/recall/F1 from the
  real eval curve), MITRE/CAPEC family knowledge, and a link to the Home globe.
  Every number is committed eval data — nothing is fabricated.
- **📜 FORENSICS** (ex-Forensic Audit) — SHA-256 Merkle-chain ledger
  (tamper-evidence) + SOC incident PDF. Incident drill-down offers a
  **per-incident forensic PDF** (select incident → generate → download).
- **📦 REPORTS** (ex-Reports & Export Hub) — the aggregated export hub
  (snapshot `.md`, incident `.md`, metrics `.csv`, timeline `.json`, ledger
  `.json`, model card).
- **Reports & Export Hub** — one-click exports: snapshot.md, incident narrative
  .md, metrics.csv, ledger.json, model card.
...
(Scapy `sniff`) and funnels them into the **same 500-packet window
accumulator** the packet path uses (`packet_features.ingest_packet` /
`finalize_window`), so a live window is **bit-identical** to a PCAP-derived
one. Capture + prediction run in background daemon threads; the UI polls a
thread-safe `LiveState` snapshot on an auto-refreshing panel.

### Two modes
- **Sniff (real capture)** — needs packet-capture privileges (root /
  `/dev/bpf0`). Without them the UI shows a clear *"Permission denied — run
  with sudo, or switch to Replay demo stream"* instead of crashing.
- **Replay (no root)** — the same engine streams the committed Friday-DDoS
  windows at a configurable cadence (0.2–10 w/s), feature-identical to real
  capture. **Recommended for judging** since any machine works.

### Live panel (SOC Command Center)
Auto-refreshing `st.fragment(run_every=1.5)`: feed status + packets + windows +
alerts + peak-risk metric cards, snapshot (pps/uptime/window-in-progress),
**live risk timeline** (threshold rule + alert markers), **live incident
...
- Dashboard live source renders with 0 exceptions (AppTest, replay + sniff).
...
## 13a. SOC workflow features (incident correlation, SOAR, reports)

### Incident correlation (`infer.correlate_incidents`)
Windows are merged into **incidents** when consecutive alert-windows lie within
a small gap (default `gap=2`). Example on the Friday-DDoS demo:

- **#1 HIGH botnet** — w36, severity 0.521, 1 window
- **#2 CRITICAL DDoS** — w38→394, severity 1.000, 357 windows (~178.5K flows)

### Attack narrative timeline
After the threat matrix, a **Gantt-style family × window** chart shows which
attack families overlapped and for how long — a single visual narrative of the
whole incident.

### SOAR course-of-action simulation (Active Defense tab)
- **Blast-radius panel**: per-family scoped containment (which hosts/ports to
  block) so the operator only cuts what's needed.
- **Auto-approve toggle**: ON → autonomous containment runs a staged timeline
  (inspect → contain → block → honeypot) with a full audit trail; OFF → the
  guardrail shows "awaiting analyst approval" before any action.
- **Hosts slider**: scales the simulated blast-radius scope.

### Per-incident SOC forensic PDF
In the incident drill-down, selecting an incident and pressing generate builds
a full offender profile and emits a **fpdf2 PDF report** (family, stage,
severity, timeline, driving features, MITRE kill-chain, CAPEC/CVE) available
for download.

### Reports & Export Hub (6th tab)
One-click exports of every artifact: `snapshot.md`, incident narrative `.md`,
`metrics.csv`, forensic `ledger.json`, and the model card — the whole submission
packaged for judges.
...
Yes — `streamlit run app.py` launches the full app. From a fresh clone, click
**Run Friday DDoS Demo** in the SOC Command Center (ships the pre-featurized demo
CSV — no large downloads). Then walk through the 6 tabs: risk timeline + RF/LSTM
comparison, explainability, What-If lab, Active Defense + SOAR blast-radius
simulation, Forensic Audit (Merkle ledger + per-incident PDF), and the
Reports & Export Hub. The Home page globe renders fully offline (three.js
inlined; 2D canvas fallback when WebGL is unavailable — lazy-loaded via the
"🌐 Load globe" button).
...
1. **Intro (15s)**: "This is NetSight — an AI-based Network Attack Forecasting
   ...
   and it can also forecast a **live traffic stream** captured straight off a
   network interface."

2. **Home + globe (15s)**: Open `streamlit run app.py`. Hit "🌐 Load globe" to
   show the animated threat-arc globe (three.js inlined — renders offline; 2D
   canvas fallback if WebGL is blocked).

3. **Run Demo (15s)**: In the SOC Command Center click **Run Friday DDoS Demo**
...
4. **Risk timeline (20s)**: "The model forecasts risk 6 windows ahead. The
   first alert fires at window 36 — well before the DDoS peaks at window 50.
   That's the lead time the operator needs."

5. **Incidents + SOAR (20s)**: "Correlation merges these into a CRITICAL DDoS
   incident. In Active Defense we set the blast radius per family and — with
   auto-approve — watch an autonomous staged containment, or gate it behind
   analyst approval."

6. **Explainability (15s)**: "The model tells you *why* it fired. Mean-imputation
   ablation shows risk dropped when we set byte_rate to its batch mean; the LSTM
   shows gradient saliency per timestep."

7. **Forensics + Reports (20s)**: "Forensic Audit keeps a SHA-256 Merkle-chain
   ledger and a per-incident PDF. Reports & Export Hub packages snapshot.md,
   metrics.csv, and the model card for judges in one click."

8. **Closing (20s)**: "Key results: cross-day AUC 0.763, forecasting AUPRC 0.877,
   median lead time 8 windows, DDoS warned 269/278. Reproducible via run_all.sh."
...
9. **Only showing a flag, not the workflow** — demo the full SOC loop: incident
   correlation → narrative timeline → SOAR blast-radius + auto-approve →
   forensic PDF → Reports & Export Hub. That's what separates a demo from a
...
| **Incident** | One or more consecutive alert-windows within a small gap (`correlate_incidents`, gap=2) |
| **Blast radius** | The scoped set of hosts/ports an action would affect; shrank per-family to contain only what's needed |
| **Auto-approve** | SOAR-mode toggle: ON runs autonomous staged containment; OFF gates actions behind analyst approval |
| **Merkle chain** | SHA-256 hash-linking in `forensic_audit_ledger.json` proving the audit trail is untampered |
| **Per-incident PDF** | fpdf2-generated SOC forensic report per drill-down incident |
| **Live Traffic Stream** | Real-time Scapy capture off a network interface feed into the same 500-packet windows as the file path (`live_sniffer.py`) |
| **Replay mode** | Streams committed demo windows at a set cadence — feature-identical live experience with no root/capture privileges |
| **LiveState** | Thread-safe snapshot (pps, uptime, packets, peak risk, incidents) that the UI polls instead of touching capture internals |
| **/dev/bpf0** | macOS packet-capture device — the OS denies non-root Scapy access; surfaced as a clear UX message |
