...
```
  ...
  │  LIVE ─────► live_sniffer.py (Scapy sniff on interface,  │   │
  │              SAME ingest_packet/finalize_window, so      │   │
  │              live windows are bit-identical) ───────────┤   │
  │                                                          ▼   │
  │                              app.py (Streamlit)             │
...
- **`live_sniffer.py`** closes the loop between the offline packet path and
  *now*: it captures packets straight off a live network interface with Scapy
  (`sniff`) and feeds the **exact same 500-packet window accumulator**
  (`packet_features.ingest_packet` / `finalize_window`), so a live window is
  **bit-identical** to one from a PCAP file. The models cannot tell the
  difference.
- **Background threads**: a daemon capture thread ingests packets; a second
  daemon thread predicts on closed windows in batches (BATCH_LIMIT=16),
  recomputes correlation every 40 windows, tracks pps/uptime/packets/peak-risk,
  and publishes a thread-safe snapshot. The UI polls via
  `LiveState.snapshot()` — never touches shared mutable state directly.
- **Replay mode (no root)**: when packet-capture privileges are unavailable
  (no `/dev/bpf0` without sudo), the same engine instead streams the
  committed Friday-DDoS windows at a configurable cadence (0.2–10 w/s) —
  feature-identical to real capture, enabling the entire live-forecasting
  experience on any machine and in cloud sandboxes.
- **Privilege UX**: sniffing without root surfaces a clear *"Permission denied:
  could not open /dev/bpf0 — run with sudo, or switch to Replay demo stream"*
  message in the live panel instead of crashing the app.
- **Live panel** (`dashboard.py`): an auto-refreshing
  `st.fragment(run_every=1.5)` renders live metric cards (feed status, packets,
  windows, alerts, peak risk), a snapshot card, a **live risk timeline** chart
  (threshold rule + alert markers), **live incident correlation**, and a live
  alert log. A "⛔ Stop live capture" button stops the session cleanly.
- **Session lifecycle**: the sidebar builds/reuses a `LiveSession` keyed on
...
## 12. Inference runtime & offline UI
...
- **`infer.py`** streams raw CICIDS CSVs in 20k-row chunks into an in-progress
  ...
  ~21 s user time. `correlate_incidents(timeline, gap=2)` then merges
  consecutive alert-windows into **incidents** (e.g. #1 HIGH botnet w36 sev
  0.521 · #2 CRITICAL DDoS w38→394 sev 1.000, 357 windows).
- **`app.py`** is the Streamlit entry point using `st.navigation` (Home + SOC
  Dashboard). It applies the app-level cyber ambience (pure CSS, since
  `<script>` does not execute in Streamlit's main document) and hosts the SOC
  ticker.
- **`home.py`** renders an animated threat-arc **globe** inside an
  `st.iframe`. The globe **inlines vendored three.js** (`static/three/`,
  r128 UMD + OrbitControls) at runtime via `_inline_js()` so it renders with
  **zero external fetches — fully offline** inside Streamlit's sandboxed iframe
  (CDN ES-module imports and `/static/` serving both fail in the sandbox). A
  **Canvas-2D rotating-planet fallback** renders when WebGL is unavailable; the
  640 KB srcdoc is **lazy-loaded** behind the "🌐 Load globe" button to avoid
  rerender lag. Home also shows live eval stats, a pipeline diagram, and a
  "for judges" metrics panel.
- **`dashboard.py`** is the **SOC Command Center**: a **Command** overview
  (telemetry strip, an inline-SVG **network-activity map** sized by real
  flagged windows, **threat/incident queues** — the threat queue pre-selects a
  window in INVESTIGATION — and a ledger-driven **event workflow** timeline)
  above six workflow centers: **DETECTION** (risk timeline with alert markers,
  alert feed, threat-matrix heatmap, attack-narrative Gantt timeline, RF-vs-LSTM
  side-by-side comparison) / **INVESTIGATION** (per-window attribution +
  counterfactual what-if) / **RESPONSE** (SOAR playbooks) / **INTELLIGENCE**
  (per-family detection, operating points, MITRE/CAPEC knowledge, link to the
  Home globe) / **FORENSICS** / **REPORTS**. Detection quality + incident
  intelligence (severity chart, drill-down + per-incident SOC **PDF** via
  `forensics_report.py`). The former "Forecaster" tab supports the one-click
  **Run Friday DDoS Demo** (pre-featurized CSV) plus CSV/PCAP upload, model
  picker, threshold slider. `st.switch_page` is called in the main script body
  (not an `on_click` callback) for reliable navigation.
- **Active Defense** (`active_defense.py`): MITRE intel, generated firewall
  rules (iptables/netsh/Cisco ACL), honeypot DNAT, plus the **SOAR blast-radius
  simulation** — per-family scoped containment, an **auto-approve toggle**
  (ON = autonomous staged timeline with audit trail; OFF = "awaiting analyst
  approval" guardrail), and a hosts slider for blast-radius scope.
...
- Seeds pinned (`random_state=42`, `np.random.seed(42)`); model artifacts,
  ...
  dataset (raw captures git-ignored; redownload from UNB to rebuild). The
  recommended demo needs nothing to download: `dataset/demo_friday_ddos_windows.csv`
  is the pre-featurized Friday-DDoS output (452 windows · 358 alerts · peak
  risk 1.000 · first alert w36), bit-identical to the full-file run — **Run
  Friday DDoS Demo** works from a fresh clone.
- Deployed UI notes: `app.py` uses `st.navigation`; `app.py`/`home.py`/
  `dashboard.py` are the routed pages. The globe iframe needs **WebGL** for the
  three.js render and transparently falls back to a **Canvas-2D** planet;
  it is lazy-loaded behind a button. Home/app ambience is pure CSS (JS-in-HTML
  does not run in Streamlit's main document; only the globe iframe runs JS).
...
- **Deliverables**: source code + README + architecture write-up (this file) +
  5-slide deck (`docs/sih26153_deck.md`) + demo video + running offline app.
...
6. **sklearn pickle version-drift** is a warning (verified valid 1.8→1.9); the
   deployed `requirements.txt` is left unpinned (`numpy/pandas/scikit-learn/
   streamlit/scapy/fpdf2`) for Cloud reproducibility, and sklearn suppresses the
   harmless `InconsistentVersionWarning`.
...
| `app.py` | Streamlit entry point (`st.navigation`) → Home + SOC Command Center; app CSS/ticker |
| `home.py` | Home: offline threat-arc globe (inlined three.js + 2D canvas fallback, lazy-load) + stats |
| `dashboard.py` | SOC Command Center: Command overview (telemetry/map/queues/event workflow) + 6 centers (DETECTION/INVESTIGATION/RESPONSE/INTELLIGENCE/FORENSICS/REPORTS) |
| `active_defense.py` | RESPONSE center: MITRE intel, firewall rules, honeypot DNAT, blast-radius + auto-approve sim |
| `forensics_report.py` | SHA-256 Merkle-chain ledger (tamper-detecting) + SOC incident PDF (fpdf2) |
...
| `static/three/` | Vendored three.js (r128 UMD) + OrbitControls — inlined into the globe for offline render |
| `run_all.sh` | One-command reproducibility pipeline |
| `requirements.txt` | Deployment dependencies (unpinned: numpy/pandas/scikit-learn/streamlit/scapy/fpdf2) |
...
| `forensic_audit_ledger.json` | SHA-256 Merkle chain for tamper evidence |
| `dataset/demo_friday_ddos_windows.csv` | Pre-featurized Friday-DDoS demo (452 windows) — Run Demo works on fresh clone |
| `docs/architecture.md` | This file — detailed master architecture |
| `docs/sih26153_deck.md` | 5-slide presentation deck |
| `docs/study_guide.md` | Comprehensive judge study guide + demo script |