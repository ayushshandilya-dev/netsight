...
- **Full offline SOC-grade UI**: animated threat-arc globe (three.js inlined —
  renders fully offline; Canvas-2D fallback when WebGL is blocked), 6-tab
...
- **SOAR course-of-action simulation**: per-family blast-radius scoping with an
  **auto-approve** toggle (autonomous staged containment + audit trail, or a
  guardrail-gated "awaiting analyst approval" mode).
...
**Offline app (`streamlit run app.py`)** — multi-page (`st.navigation`)
- **Home**: animated threat-arc globe (three.js inlined; offline; 2D canvas
  fallback; lazy-loaded via "Load globe"), eval stats, "for judges" panel.
- **SOC Command Center — one-click Run Friday DDoS Demo** (ships pre-featurized
  CSV — no download) across **six workflow centers**: 🔭 DETECTION (RF/LSTM
  picker, threshold, risk timeline, **RF-vs-LSTM comparison**) → 🔬
  INVESTIGATION (attribution + counterfactual what-if) → ⚡ RESPONSE (**SOAR
  blast-radius + auto-approve sim**, firewall rules, honeypot DNAT) → 🌐
  INTELLIGENCE (per-family detection, operating points, MITRE/CAPEC) → 📜
  FORENSICS (**Merkle ledger + per-incident PDF**) → 📦 REPORTS (`snapshot.md`,
...
**Roadmap**: real CICIDS2017 PCAP validation of the packet path; live-interface
validated on a multi-hour real capture with ground truth; extend approved
