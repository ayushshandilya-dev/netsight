...
# 🌐 live traffic stream: Scapy interface capture → real-time forecasts
python3 live_sniffer.py en0         # sniff live (needs root / sudo)
python3 live_sniffer.py --replay dataset/demo_friday_ddos_windows.csv --rate 10

# upload-and-forecast demo (accepts CSV, PCAP, or a live traffic stream)
...
live interface  → live_sniffer.py (Scapy sniff, 500-packet windows)
                ...
                → deployed RF / LSTM forecasters streamed live
                → live risk timeline, alerts, live incident correlation
```
- **Sniff mode**: `live_sniffer.py` captures packets straight off a network
  interface with Scapy (`sniff`), reusing `packet_features.ingest_packet /
  finalize_window` so a live window is **bit-identical** to a file-driven one.
  Capture + prediction run in background daemon threads; the dashboard polls a
  thread-safe `LiveState` snapshot on an auto-refreshing
  `st.fragment(run_every=…)` panel (live metrics, live risk chart, live alert
  log, live incident correlation).
- **Needs packet-capture privileges** (root / BPF on macOS-Linux). Without
  them the UI surfaces a clear "permission denied — run with sudo" message
  instead of crashing.
- **Replay mode (no root)**: selects the committed Friday-DDoS windows and
...
3. Verify: Home page (animated threat-arc globe — works offline & without
   WebGL via a 2D canvas fallback) + SOC Command Center (six workflow centers:
   🔭 DETECTION + 🔬 INVESTIGATION + ⚡ RESPONSE + 🌐 INTELLIGENCE +
   📜 FORENSICS + 📦 REPORTS, with a Command overview: telemetry strip,
   network-activity map, threat/incident queues, event workflow) — across CSV
   upload, PCAP upload, RF/LSTM toggle, and the demo artifact (Friday DDoS
   recommended)
...
| `dashboard.py` | SOC Command Center: Command overview (telemetry strip, network-activity map, threat/incident queues — the threat queue pre-selects a window in INVESTIGATION — and a ledger-driven event workflow timeline) + six workflow centers: DETECTION (risk timeline, alert feed, threat matrix, narrative, RF-vs-LSTM engine comparison, timeline scrubber) / INVESTIGATION (per-window attribution + counterfactual what-if) / RESPONSE (MITRE intel, firewall rules, SOAR blast-radius & auto-approve sim, honeypot) / INTELLIGENCE (per-family detection, operating points, MITRE/CAPEC knowledge, globe link) / FORENSICS (Merkle ledger + per-incident PDF) / REPORTS (export hub); detection quality + incident intelligence + SOC PDF per incident — plus the 🌐 **Live Traffic Stream** data source (live interface capture or replay) with an auto-refreshing live panel and REPLAY/LIVE-CAPTURE mode pill |
...
- [x] Thread-safe `LiveState` snapshots: pps, elapsed, packets, peak risk, live incidents
- [x] Auto-refreshing live panel in SOC Command Center (`st.fragment(run_every)`): metrics, live risk chart, live alert log, live incident correlation
- [x] Clean permission-denied UX when sniffing without root (`/dev/bpf0` / sudo)
- [x] AppTest-verified: live source renders with 0 exceptions (replay + sniff paths)
...
- [ ] Real-interface validated on a multi-hour capture with ground truth
...
- **Shipped demo dataset**: `dataset/demo_friday_ddos_windows.csv` (39 KB) is the
  ...
  large downloads. Click **Run Friday DDoS Demo** in the SOC Command Center.
