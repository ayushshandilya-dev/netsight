...
def new_tweets():
    """Fresh in-progress window accumulator state."""
    return {
...
def reset_tweets(tweets):
    tweets.clear()
    tweets.update(new_tweets())


def ingest_packet(tweets, pkt):
    """Apply one packet to an in-progress window accumulator — the exact
    per-packet math formerly inside PacketWindowIterator. Shared by the offline
    PCAP path and the live sniffer so live windows are bit-identical to
    file-driven ones."""
    if IP not in pkt:
        tweets["n"] += 1
...
def finalize_window(tweets):
    """Close the current 500-packet window and compute its aggregate feature
    dict (identical math to packet_features' historical finish()): returns None
    on an empty window, else the feature dict, then resets the accumulator."""
    n = tweets["n"]
    if n == 0:
        return None
    ...
    out = {
        "packet_rate": n / dur,
        "byte_rate": tweets["ip_bytes"] / dur,
        "unique_dst_ips": len(tweets["dst_ips"]),
        "unique_dst_ports": len(tweets["dst_ports"]),
        "syn_ack_ratio": (tweets["syn_noack"] + 1.0) / (tweets["syn_ack"] + 1.0),
        "avg_pkt_size": sum(sizes) / len(sizes) if sizes else 0.0,
        "dst_port_entropy": infer._entropy(tweets["dst_ports"]),
        "failed_conn_rate": tweets["rst"] / tcp_n,
        "fwd_psh_rate": tweets["psh"] / tcp_n,
        "avg_flow_duration": (sum(flow_durs) / len(flow_durs)) if flow_durs else 0.0,
        "ttl_mean": sum(ttls) / len(ttls) if ttls else 0.0,
        "ttl_std": (sum((t - sum(ttls) / len(ttls)) ** 2 for t in ttls) /
                    len(ttls)) ** 0.5 if len(ttls) > 1 else 0.0,
        "tcp_win_mean": sum(wins) / len(wins) if wins else 0.0,
        "tcp_win_std": (sum((w - sum(wins) / len(wins)) ** 2 for w in wins) /
                        len(wins)) ** 0.5 if len(wins) > 1 else 0.0,
        "frag_ratio": tweets["frag"] / n,
        "df_ratio": tweets["df"] / n,
        "syn_only_rate": tweets["syn_noack"] / n,
        "icmp_ratio": tweets["icmp"] / n,
        "udp_ratio": tweets["udp"] / n,
        "retrans_ratio": tweets["retrans"] / n,
        "distinct_src_ips": len(tweets["src_ips"]),
    }
...
class PacketWindowIterator:
    ...
    def __iter__(self):
        ...
        with PcapReader(self.path) as reader:
            for pkt in reader:
                ingest_packet(tweets, pkt)
                if tweets["n"] >= self.window_packets:
                    w = finalize_window(tweets)
                    if w:
                        yield w
        w = finalize_window(tweets)
        if w:
